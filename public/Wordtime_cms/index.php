...
if ($p === 'comment-add' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($err !== null) { wt_set_option('last_comment_error', $err); } else { wt_set_option('last_comment_error', ''); }
    header('Location: ' . $back . (strpos($back, '?') === false ? '?' : '&') . 'cm=' . ($err === null ? 'ok' : 'err'));
    exit;
}

/* ── Рендер сайта через тему ──────────────────────────────────────── */
$GLOBALS['wt_view'] = array('route' => $p, 'segments' => $segments);
require wt_theme_file('index.php');
