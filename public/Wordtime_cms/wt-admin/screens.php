...
function wt_screen_updates() {
    ...
    $changelog = array(
        array('1.0.5', 'сегодня', array('Полная PHP-консоль с выпадающими меню: 15+ разделов', 'Редактор тем и загрузка плагинов из консоли', 'GD-оптимизация изображений при загрузке', 'SEO-шаблоны title/description и экран Sitemap', 'Управление меню сайта, рубриками и метками')),
    ...
    $mods = array(
        ...
        array('wt-content/themes/wordtime-twenty', 'Стартовая тема'),
        array('wt-cron.php', 'Асинхронная очередь'),
    );
    foreach ($mods as $m) {
        $ok = file_exists(WT_ROOT . '/' . str_replace('wt-content/themes/wordtime-twenty', 'wt-content/themes/wordtime-twenty/index.php', $m[0]));
        echo '<tr><td><code>' . esc($m[0]) . '</code><br><span style="color:var(--mut);font-size:12px">' . esc($m[1]) . '</span></td>';
        echo '<td style="width:90px">' . ($ok ? '<span class="badge b-ok">на месте</span>' : '<span class="badge b-red">отсутствует</span>') . '</td></tr>';
    }
    echo '</table></div></div>';
    wt_shell_close(); exit;
}
...
function wt_screen_categories() {
    ...
    foreach ($cats as $c) {
        echo '<tr><td><b>' . esc($c) . '</b></td><td><span class="badge b-mut">' . (isset($counts[$c]) ? $counts[$c] : 0) . '</span></td>';
...
function wt_screen_tags() {
    ...
    if (count($tags) === 0) { echo '<div class="empty">' . wt_icon('tag', 28) . '<br>Меток пока нет — добавляйте их в редакторе записей.</div></div>'; }
        ...
        foreach ($tags as $t => $n) echo '<span class="swatch" style="cursor:default"><i style="background:linear-gradient(150deg,var(--teal),#0b6e63)"></i>' . esc($t) . ' <b style="color:var(--mut)">× ' . $n . '</b></span>';
        echo '</div></div>';
    }
    wt_shell_close(); exit;
}
...
function wt_screen_profile() {
    ...
    echo '<p style="color:var(--mut);font-size:13px;margin:14px 0 0;line-height:1.7">Каждый вход в консоль подтверждается шести-значным кодом из письма. После 5 неудачных попыток вход блокируется на 60 секунд.</p></div>';
    echo '</div>';
    wt_shell_close(); exit;
}
...
function wt_screen_menus() {
    ...
    if (count($m) === 0) echo '<div class="empty">' . wt_icon('menu', 28) . '<br>Меню пустое — добавьте первый пункт справа.</div>';
        ...
        foreach ($m as $i => $it) {
            ...
            echo '<td><input type="text" name="targets[]" value="' . esc_attr($it['url']) . '" placeholder="/ или ?p=page:o-sajte"></td>';
    ...
    echo '</div>';
    echo '<div><div class="card"><h2>Добавить пункт</h2>';
    wt_form_open(array('action' => 'menu-add'));
    echo '<label>Название</label><input type="text" name="label" required placeholder="О проекте">';
    echo '<label>Ссылка</label><input type="text" name="target" required placeholder="/ или ?p=page:o-sajte">';
    echo '<p style="margin:16px 0 0"><button class="btn" type="submit">' . wt_icon('plus', 15) . 'Добавить</button></p></form></div>';
    ...
    foreach (array_slice($m, 0, 6) as $it) echo '<span style="color:#9fc0c5;font-size:12.5px;font-weight:600">' . esc($it['label']) . '</span>';
...
function wt_screen_plugins() {
    ...
    wt_shell('plugins', 'Плагины', 'Установленные расширения ядра — подключение через хуки wt_add_action / wt_add_filter');
...
function wt_screen_plugin_new() {
    ...
    echo '<div class="card"><h2>Каркас плагина</h2><pre class="codebox" style="max-height:280px">&lt;?php
/**
 * Plugin Name: Мой плагин
 * Description: Пример расширения Wordtime
 */

// Свой заголовок на каждой странице
wt_add_action('wt_head', function () {
    echo "&lt;meta name=\"generator\" content=\"Мой плагин\"&gt;";
});

// Фильтр заголовков (как the_title в WP)
wt_add_filter('wt_title', function ($title) {
    return $title; // изменяем и возвращаем
});

// Фоновая задача через очередь (cron)
...
function wt_screen_export() {
    ...
    echo '<button class="btn dark" type="submit">' . wt_icon('db', 15) . 'Сделать дамп</button></form></div>';
    echo '</div>';
...
function wt_screen_images() {
    ...
    echo '</p></div></form>';
    echo '<div class="card"><h2>Как это работает</h2><p style="color:var(--mut);font-size:13.5px;margin:6px 0 0;line-height:1.8">При каждой загрузке через «Медиафайлы» ядро проверяет ширину изображения и, если она больше лимита, пересобирает файл в <code>' . (int)wt_option('img_max_width', 1920) . ' px</code> с качеством <code>' . (int)wt_option('img_quality', 82) . '%</code>. В журнал попадает отметка об оптимизации. SVG и PDF не трогаются.</p></div>';
    wt_shell_close(); exit;
}
...
function wt_screen_sitemap() {
    $xml = wt_sitemap_xml();
    $url = (isset($_SERVER['REQUEST_SCHEME']) ? $_SERVER['REQUEST_SCHEME'] : 'https') . '://' . (isset($_SERVER['HTTP_HOST']) ? $_SERVER['HTTP_HOST'] : 'localhost') . wt_url('sitemap.xml');
...
function wt_screen_seo() {
    ...
    echo '<p style="margin:0;font-size:13px;color:#4d5156;line-height:1.5">' . esc($exDesc) . '</p>';
    echo '</div>';
    ...
    echo '<div class="kv"><b>Длина description</b><span class="badge ' . (mb_strlen($exDesc) <= 160 ? 'b-ok' : 'b-amber') . '">' . mb_strlen($exDesc) . ' / 160</span></div>';
    echo '</div>';
    echo '<div class="card" style="margin:0"><h2>Где это в коде</h2><pre class="codebox" style="max-height:170px">&lt;head&gt;
  &lt;meta charset="utf-8"&gt;
  ...
  …
&lt;/head&gt;</pre>
    <p style="color:var(--mut);font-size:13px;margin:10px 0 0">Теги ставит функция <code>wt_head()</code> темы — title и description находятся в первых строках <code>&lt;head&gt;</code>, как любят поисковики.</p></div>';
    echo '</div></div>';
    wt_shell_close(); exit;
}
...
function wt_screen_hosting() {
    ...
    echo '<p style="margin:7px 0 0">Сайт установлен и отвечает. Ниже — требования, конфиг nginx и шаги для переноса копии на другой хостинг.</p></div>';
    ...
    else echo '<div class="empty">' . wt_icon('file', 26) . '<br>Файл nginx-wordtime.conf не найден в корне сайта — он есть в свежем дистрибутиве.</div>';
    echo '</div></div></div>';
...
function wt_screen_settings() {
    ...
    if ($tab === 'general') {
        ...
        echo '<label style="display:flex;gap:10px;align-items:center;cursor:pointer;margin-top:14px"><input type="checkbox" name="login_rounded" value="1" style="width:auto" ' . (wt_option('login_rounded', true) ? 'checked' : '') . '> Скруглённая карточка входа</label>';
