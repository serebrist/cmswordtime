...
function wt_admin_css() {
    ...
    echo '.side{width:254px;flex:none;position:sticky;top:0;height:100vh;display:flex;flex-direction:column;background:linear-gradient(170deg,#071b21 0%,#0b2831 55%,#0d3039 100%);color:#cfe4e6;z-index:40}';
    ...
    echo '.nav a.it{display:flex;align-items:center;gap:11px;padding:9px 11px;margin:2px 0;border-radius:10px;color:#a9c6ca;font-weight:600;font-size:13.8px;position:relative;transition:background .16s,color .16s;cursor:pointer}';
    echo '.nav a.it svg{opacity:.75;flex:none;transition:opacity .16s}';
    echo '.nav a.it:hover{background:rgba(20,184,166,.1);color:#fff}';
    echo '.nav a.it:hover svg{opacity:1}';
    ...
    echo '.nav .flychev{margin-left:auto;opacity:.5;transition:transform .18s,opacity .18s}';
    ...
    echo '.sub a::before{content:"";width:5px;height:5px;border-radius:99px;background:#35626d;flex:none;transition:.14s}';
    ...
    echo '.swatch{display:inline-flex;gap:6px;align-items:center;padding:4px 10px;border:1px solid var(--line);border-radius:99px;background:#fff;font:600 12px var(--body);cursor:pointer;transition:.15s}';
    echo '.swatch:hover{border-color:var(--teal)}';
    echo '.swatch i{width:16px;height:16px;border-radius:5px;border:1px solid rgba(0,0,0,.1)}';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $act !== '') {
    ...
    switch ($act) {
        ...
        case 'theme-file-save': {
            ...
            $path = wt_theme_dir() . '/' . $f;
            if (@file_put_contents($path, (string)$_POST['content']) !== false) {
        ...
        case 'menu-del': {
            $i = (int)$_POST['idx'];
            $m = wt_option('site_menu', array()); if (!is_array($m)) $m = array();
            if (isset($m[$i])) { wt_log('Пункт меню «' . $m[$i]['label'] . '» удалён'); unset($m[$i]); wt_set_option('site_menu', array_values($m)); }
            wt_cache_flush();
            $_SESSION['wt_flash'] = array('ok', 'Пункт меню удалён.'); wt_redirect_to('menus');
        }
        case 'menu-move': {
            $i = (int)$_POST['idx']; $dir = (int)$_POST['dir'];
            $m = wt_option('site_menu', array()); if (!is_array($m)) $m = array();
            $j = $i + $dir;
            if (isset($m[$i]) && isset($m[$j])) { $tmp = $m[$i]; $m[$i] = $m[$j]; $m[$j] = $tmp; wt_set_option('site_menu', $m); wt_cache_flush(); }
            wt_redirect_to('menus');
        }
        case 'settings-save': {
        ...
        case 'cache-clear': {
            ...
            $_SESSION['wt_flash'] = array('ok', 'Кеш сайта очищен.'); wt_redirect_to(isset($_POST['back']) && preg_match('/^[a-z-]+$/', (string)$_POST['back']) ? (string)$_POST['back'] : 'settings&tab=cache');
        }
        case 'perf-save': {
        ...
        case 'login-style': {
            ...
            wt_set_option('login_rounded', isset($_POST['login_rounded']));
...
function wt_menu_active($target) {
    ...
    if ($tb === '' && $tab !== '' && $target !== 'settings' && strpos($target, 'settings') === 0) return false;
...
function wt_shell($pageKey, $title, $sub) {
    ...
    foreach ($menu as $sec => $items) {
        ...
        foreach ($items as $it) {
            ...
            $cls = $on || $groupOn ? 'on' : '';
            echo '<a class="it ' . $cls . '" href="' . esc_attr(wt_admin_url('&page=' . $it['k'])) . '">' . wt_icon($it['i']) . '<span>' . $it['l'] . '</span>';
            ...
            echo '</a>';
            if (isset($it['fly'])) {
                echo '<div class="sub" data-sub="' . esc_attr($it['k']) . '">';
                echo '<div class="cap">' . $it['l'] . '</div>';
                foreach ($it['fly'] as $f) {
                    $fon = wt_menu_active($f['t']);
                    echo '<a class="' . ($fon ? 'on' : '') . '" href="' . esc_attr(wt_admin_url('&page=' . $f['t'])) . '">' . $f['l'] . '</a>';
                }
                echo '</div>';
            }
        }
...
function wt_shell_close() {
    ...
    echo 'a.addEventListener("click",function(e){if(window.matchMedia("(hover:none)").matches&&sub.querySelectorAll("a").length){e.preventDefault();var was=sub.classList.contains("show");subs.forEach(function(s){s.classList.remove("show");});if(!was)show();}});';
    echo '});';
